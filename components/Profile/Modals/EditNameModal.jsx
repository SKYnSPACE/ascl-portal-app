...
export default function EditNameModal({ props }) {
  ...
  const [ editName, { loading, data, error }] = useMutation("/api/profile/edit/name");
  ...
  useEffect(() => {
    if (data?.ok) {
      setMessage(
        { type: 'success', title: 'Successfully Saved!', details: 'User name modified. Wait for the page reload.', }
      )
      setIsNotify(true);
      reset();
