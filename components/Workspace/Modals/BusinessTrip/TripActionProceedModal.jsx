...
export default function TripActionProceedModal({ props }) {
  const { modal, isModalOpen, setIsModalOpen, isNotify, setIsNotify, message, setMessage, selectedAction } = { ...props };

  const [finishAction, { loading: finishActionLoading, data: finishActionData, error: finishActionError }] = useMutation("/api/workspace/businessTrip/finish");
  const [withdrawAction, { loading: withdrawActionLoading, data: withdrawActionData, error: withdrawActionError }] = useMutation("/api/workspace/businessTrip/withdraw");
  ...
  useEffect(()=>{
    ...
    if (+totalExpense == +selectedAction?.expense?.replaceAll(",",""))
    setCaution(null);
    else{
    setCaution("Caution: Total expense doesn't match with your original request. Please check again.");
  }
  },[totalExpense])
  ...
  const onClickFinished = () => {
    if (!selectedAction) return;
    if (finishActionLoading) return;
    
    trigger();
    if(isFormValid){
      console.log({selectedAction,
      response:"FINISHED", ...getValues()})

      finishAction({selectedAction,
        response:"FINISHED", ...getValues()});
    }
    else{
  ...
  useEffect(() => {
    if (finishActionData?.ok) {
      setMessage(
        { type: 'success', title: 'Finished!', details: 'Successfully reported. Wait for the page reload.', }
      )
      setIsNotify(true);
      setIsModalOpen(false);
    }

    if (finishActionData?.error) {
      switch (finishActionData.error?.code) {
        case 'P1017':
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="w-full overflow-hidden bg-white shadow rounded-lg border">
              <div className="px-4 py-3 sm:px-4">
                <h3 className="text-lg font-medium leading-6 text-gray-900">[{selectedAction?.startDate} ~ {selectedAction?.endDate}] {selectedAction?.destination}</h3>
              ...
              <div className="border-t border-gray-200 px-4 py-2 sm:p-0">
                ...
                <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-4">
                  ...
                  <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-4">
                    ...
                    <dd className="mt-0 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{selectedAction?.expense}{selectedAction?.currency}</dd>
                  </div>
                  <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-4">
            ...
            <div className="w-full">
              <label htmlFor="title" className="text-sm font-semibold">
                Total Expense (ACTUAL & EXACT expenses, 실 사용 금액)
              </label>
                  ...
                  render={({ field: { onChange, name, value } }) => (
                    <NumericFormat
                      ...
                      onValueChange={(target) => {
                        onChange();
                        setValue("totalExpense", target.floatValue);
                      }}
                    />
                  )}
                />
